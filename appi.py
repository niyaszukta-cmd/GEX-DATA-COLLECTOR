...
# ── Streamlit Cloud vs local: use writable path ─────────────────────────────────
...
# Override collector paths if needed
if COLLECTOR_OK:
...
with t2:
    ...
    with cb:
        # Show recent log
        failed = conn.execute(
            "SELECT trade_date,error_msg FROM download_log WHERE status='error' LIMIT 10"
        ).fetchall()
        if failed:
    ...
    recent = conn.execute("""
        SELECT trade_date, status, rows_stored
        FROM download_log ORDER BY trade_date DESC LIMIT 15
    """).fetchall()
    if recent:
...
with t3:
    ...
    ohlcv_cov = conn.execute("""
        SELECT symbol, MIN(trade_date), MAX(trade_date), COUNT(*)
        FROM index_ohlcv GROUP BY symbol
    """).fetchall()
...
with t4:
    ...
    **What gets computed per trading day:**
    ...
    gex_cov = conn.execute("""
        SELECT symbol, MIN(trade_date), MAX(trade_date), COUNT(*),
               AVG(net_gex_total) as avg_gex,
               COUNT(CASE WHEN gex_regime='POSITIVE' THEN 1 END) as pos_days,
               COUNT(CASE WHEN gex_regime='NEGATIVE' THEN 1 END) as neg_days
        FROM gex_daily_summary GROUP BY symbol
    """).fetchall()
    if gex_cov:
...
with t6:
    ...
    with ptab4:
        ...
        dates = conn.execute("""
            SELECT DISTINCT trade_date FROM gex_per_strike
            WHERE symbol=? ORDER BY trade_date DESC LIMIT 20
        """, (sym3,)).fetchall()
